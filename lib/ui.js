...
io.on('connect', (client) => {
  ...
  client.on('CLIENT_CMD_RUN', function (command) {
    ...
    let cpob = require('child_process').spawn(command, [], {
      shell: true,
      encoding: 'buffer',
  ...
  client.on('CLIENT_EXIT_CMD', function () {
    console.log('客户端 - 退出CMD命令', CMD_PID)
    func.exitCmd(CMD_PID)
  })
  ...
  client.on('CLIENT_GEN_PROJECT', function (json) {
    console.log('客户端 - 构建项目', json.peth, json.name, json.gitpath)
    if (!json)
      return
    func.genProject(json)
  })
